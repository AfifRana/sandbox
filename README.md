...
- [ ] Distributed gateway rate limiting: use a Redis-backed token bucket shared by all gateway replicas; return `429 Too Many Requests` and `Retry-After`; limit login by IP plus normalized username hash, anonymous reads by IP, authenticated writes by JWT subject, and flash-sale reservations by subject plus sale/product; exclude health/metrics; E2E-verify burst/refill, two-user fairness, cross-replica shared quota, and route-specific Redis-outage policies
- [ ] Full regression revalidation: after the Saga, CI/CD, concurrency, flash-sale, and rate-limiting milestones, rerun Maven verification, focused PIT, Docker Compose and Kubernetes gateway E2E flows, observability checks, and k6 workloads; update affected guides with actual results
...
- [ ] Final regression revalidation: after the OAuth2/OIDC and resource-efficiency milestones, repeat Maven verification, focused PIT, Docker Compose and Kubernetes gateway E2E flows, observability checks, k6 workloads, and the measured resource workload; update affected guides with actual results
- [ ] ADRs: document Kafka versus RabbitMQ, the transactional outbox, Saga orchestration, flash-sale concurrency/consistency, distributed rate limiting, OAuth2/OIDC, and resource-efficiency trade-offs after those implementations are complete
