...
- [ ] Distributed gateway rate limiting: add Redis-backed token buckets shared across replicas; return `429 Too Many Requests` with `Retry-After`; use IP plus normalized username hash for login, IP for anonymous reads, JWT subject for authenticated writes, and subject plus sale/product for flash-sale reservations. Exclude health/metrics; E2E-verify burst/refill, two-user fairness, shared cross-replica quotas, route-specific Redis outage policy (fail closed for login/flash sale; documented policy for catalog reads), and allowed/rejected/backend-error metrics
- [ ] Full regression revalidation after the Saga, CI/CD, concurrency, flash-sale, and rate-limiting milestones: run the complete Maven verification suite, focused PIT for order/payment/product, Docker Compose end-to-end lifecycle, Kubernetes Helm gateway lifecycle, observability checks, and the k6 workload; update every affected E2E guide with the actual results
- [ ] OAuth2/OIDC authorization server: replace the custom JWT issuer with a mature standards-based implementation; add authorization-code flow with PKCE, registered clients, OIDC discovery/UserInfo, persistent signing keys with safe rotation, refresh-token rotation, and issuer/audience/scope validation. E2E-verify authorization, expiry, insufficient scope, refresh rotation and reuse rejection, key rotation, restart persistence, Docker Compose, and Kubernetes paths
